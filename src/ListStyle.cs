...
internal static class ListStyle
{
    ...
    /// <summary>
    /// A surface colour as it should be laid over the background picture. <paramref name="solid"/>
    /// is the alpha that surface uses when the setting is at full; the user's choice scales it down
    /// so more of the picture shows through. Without a picture the colour is left opaque, since
    /// there would be nothing behind it but the flat list.
    /// </summary>
    public static Color OverBackground(Color colour, int solid)
    {
        if(!Background.HasImage) return colour;

        var percent = Math.Clamp(Settings.Current.CardOpacity, Settings.MinCardOpacity, Settings.MaxCardOpacity);
        return Color.FromArgb((int)Math.Round(solid * (percent / 100.0)), colour);
    }
