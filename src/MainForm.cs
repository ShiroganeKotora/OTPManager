...
internal sealed class MainForm : Form
{
    ...
    private void ApplyTheme()
    {
        ...
        // The list's own scrollbar is drawn by Windows, so it has to be told about the theme too.
        Theme.ApplyToScrollBars(_list);
    ...
    private void Tick()
    {
        ...
        // The slideshow runs off this timer rather than one of its own, so nothing is left ticking
        // once the window is gone.
        Background.TickCycle();
    }
    ...
    private void LayoutItems()
    {
        ...
        // Resizing the rows can make the vertical scrollbar appear or vanish, which changes the
        // client width again and asks for another pass. Dropping that second request leaves the
        // rows as wide as the panel used to be, and a horizontal scrollbar shows up to reach them.
        // So remember the request and run again - but only while the width is still moving, since
        // a scrollbar right at the threshold could otherwise flip back and forth forever.
        if(_layingOut) { _layoutPending = true; return; }
        _layingOut = true;

        try
        {
            var lastWidth = int.MinValue;
            for(var pass = 0; pass < 3; pass++)
            {
                _layoutPending = false;

                var width = _list.ClientSize.Width;
                if(width == lastWidth) break;
                lastWidth = width;
