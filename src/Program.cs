...
internal static class Program
{
    ...
    private static void Main(string[] args)
    {
        ...
        // Has to happen before the first window exists: it decides how Windows draws the parts the
        // app never touches, scrollbars above all.
        Theme.ApplyColorMode();
    ...
    /// <summary>
    /// Every code width the app offers, over a stretch of time. Ten digits used to be on that list
    /// and could not work: truncation leaves 31 bits, and the modulo for ten digits overflows the
    /// int it was computed in, so the result was not a ten-digit code at all.
    /// </summary>
    private static int DigitWidths()
    {
        var secret = Base32.Decode("JBSWY3DPEHPK3PXP");
        var failed = 0;

        for(var digits = Totp.MinDigits; digits <= Totp.MaxDigits; digits++)
        {
            var bad = 0;
            for(long time = 0; time < 5000; time += 37)
            {
                var code = Totp.Generate(secret, time, 30, digits, "SHA1");
                if(code.Length != digits || !code.All(char.IsAsciiDigit)) bad++;
            }

            var good = bad == 0;
            if(!good) failed++;
            Console.WriteLine(good ? $"OK   {digits} digit codes" : $"FAIL {digits} digit codes: {bad} malformed");
        }
        return failed;
    }
